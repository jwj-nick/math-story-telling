...
> ⚠️ **발화 텍스트 = 한글 음차 필수 (unit-04 학습, 2026-05-30)**: TTS가 **로마자 letter("x")와 큰 숫자("84")를 한국어로 못 읽어 발음이 완전히 깨짐**(영어 발음 혼입·잡음). → 발화 jsonl/txt 에서: **letter → 한글 음차**("x"→"엑스", "y"→"와이", "r"→"알"), **숫자 → 한글 단어**("84"→"여든네 살" 또는 "팔십사", "628"→"육백이십팔"). ⚠️ **자막(drawtext)은 시각 기호 그대로 유지**("x = 84"). 즉 *보이는 건 x, 들리는 건 엑스*. 분수도 "6분의 1"처럼 한글(이미 적용 중).

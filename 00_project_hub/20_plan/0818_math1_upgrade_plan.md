...
| Phase | 내용 | 상태 |
|---|---|---|
| **P0** | 공유 엔진 스캐폴딩(`concept.js`/`concept.css` math1판) + 허브 `index.html` 개편 | ✅ 완료 (2026-08-19) |
| **P1 ★파일럿** | 단원07(기본도형) 전체 재설계: `u7.html`+`g7.html`+`p7.html`+`d7.html` | ✅ **완료 (2026-08-19)** — 프린트 재생성만 남음(아래 참조) |
| P1.5 | 파일럿 리뷰(Nick) — 압축 강도·톤·놀이터 난이도 확정 | ⏳ Nick 검수 대기 |
| P2 | 2학기 나머지(08~13) 순차 재설계 — 단원13(자료의 정리와 해석)은 놀이터 아이디어 신규 브레인스토밍 필요 | — |
| P3 | 1학기(01~06) 순차 재설계 | P2 완료 후 |
| P4 | 허브 통합(기존 `math1/index.html` 카드 → `app1/` 링크로 전환) + 전체 감사(`/se-audit-app`) + 배포 마감 | — |
| P1-프린트 | 단원07 프린트를 새 콘텐츠(u7/g7/p7/d7)에 맞춰 `/se-print-sheet`로 재생성 | ⏳ 대기 (P1.5 승인 후) |

**P1이 게이트다.** 파일럿에서 압축 강도(29개→1개로 줄이면서 얼마나 남기는지)와 놀이터 톤이 맞는지 확인한 뒤 12단원으로 확산한다 — 트랙A/B 때와 동일 원칙.

### P1 실제 구현 요약 (2026-08-19)
- 아키텍처: `40_grades/middle/math1/app1/`(소스) ↔ `mid1/math1/app1/`(배포, math1의 기존 중첩 관례를 따름 — math2/app2와 달리 flat이 아님).
- 엔진: `concept.js`/`concept.css` 그대로 포크, `window.MJ1` 네임스페이스, localStorage 키 `m1theme`/`m1progress`(math2의 `m2*`와 충돌 방지), `UNITS` 13개(색상 토큰 신규 배정).
- 탭 체계: 개념(u)→놀이터(g)→연습(p)→심화탐구(d) 4탭. x(PhET)·dp(심화문제)는 1차 제외(계획서 §2-2 결정 그대로).
- 단원7 콘텐츠: u7(8섹션, 각도/맞꼭지각 슬라이더 신규 구현), g7(4위젯 — 위치관계판별사/각짝꿍찾기/맞꼭지각감별사/오류탐정), p7(7유형×3=21문제, 기존 29개 파일 압축), d7(꺾인각+보조선 심화 탐구, 기존 3개 deep_*+explore 대체).
- **⚠️ 배포 사고 1건 발견·복구**: 소스의 기존 `math1/index.html`이 `20_packages/design-system`(배포 repo엔 없는 경로)을 참조하는 오래된 미완성 리디자인이었음 — 배포본과 소스가 이미 어긋나 있던 상태(이번 세션 이전부터). 이를 모르고 배포에 그대로 덮어썼다가 발견 즉시 git history로 실제 배포본 복원 + 배너만 추가로 정정. **교훈: 처음 만지는 파일은 덮어쓰기 전에 배포본과 diff 확인.**

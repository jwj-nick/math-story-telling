---
name: se_unit_review
description: 단원 1개를 Nick과 함께 체계적으로 둘러보고 갭(missing/부적합)을 발견·기록하고 어떤 skill로 보완할지 결정한다. 라운드 기반 대화로 진행. 호출 예시 — "/se_unit_review unit01", "/se_unit_review 01_소인수분해".
---

# /se_unit_review — 단원 상세 리뷰 Skill (v0.1, draft)
...
```
/se_unit_review [단원번호 또는 단원명]
/se_unit_review unit01
/se_unit_review 01_소인수분해
```
...
- 디자인 개선: Edit 직접 or `/se_ncc_audit_app`
- 수학 오류: `/se_ncc_audit_math`
...
| `type_NN_app.html` 없음 | `/se_math_practice` |
| `deep_*.html` 없음 | `/se_type_explorer` |
| `story.html` 없음 | `/se_story_write` + HTML 변환 (orchestrator) |
| `concepts.html` 없음 | se_agent_unit_orchestrator (전용 skill 없음) |
| 수학 오류 | `/se_ncc_audit_math` |
| 디자인 위반 | `/se_ncc_audit_app` |
| 개념 수준 부적합 | `/se_ncc_audit_concept` |
| 문제 커버리지 미흡 | `/se_ncc_audit_problem` |
| 스토리 부적합 | `/se_ncc_audit_story` |
...
- **원래 의도**: `se_math_error_note` skill + `APP_PRINCIPLES.md` page-0~N 패턴

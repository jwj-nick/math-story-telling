...
(function(){
  ...
  /* ── ⑥ 자료 정리 — 13단원 수시평가 대비 (계급값·도수·상대도수) ── */
  var DS={
    기초:function(){
      var t=ri(1,5);
      if(t===1){ // 계급값
        var s1=pick([5,10,20]), lo=pick([0,10,20,30,40,50,60,70,140,150,160]);
        var hi=lo+s1, mid=(lo+hi)/2;
        return {q:eq('"'+lo+' 이상 '+hi+' 미만" 계급의 계급값은?'), type:'num', ans:mid,
          exp:'계급값은 가운데 값이에요. ('+lo+' + '+hi+') ÷ 2 = '+mid+'이에요.'};
      }
      if(t===2){ // 계급의 크기
        var s2=pick([2,5,10,20]), lo2=pick([0,10,20,30,40,100,140,150]);
        return {q:eq('"'+lo2+' 이상 '+(lo2+s2)+' 미만" 계급의 계급의 크기는?'), type:'num', ans:s2,
          exp:'계급의 크기는 그 구간의 폭이에요. '+(lo2+s2)+' − '+lo2+' = '+s2+'예요.'};
      }
      if(t===3){ // 도수분포표 빈칸
        var a=ri(3,9), b=ri(3,9), c=ri(3,9), x=ri(2,9), tot=a+b+c+x;
        return {q:eq('도수가 '+a+', '+b+', '+c+', ?이고 도수의 합이 '+tot+'일 때 ?는?'), type:'num', ans:x,
          exp:'나머지 도수의 합이 '+(a+b+c)+'이니까 '+tot+' − '+(a+b+c)+' = '+x+'이에요.'};
      }
      if(t===4){ // 상대도수
        var pr=pick([[20,4],[20,5],[25,5],[40,6],[40,10],[50,10],[50,15],[20,3]]);
        var N=pr[0], f=pr[1], r=Math.round(f/N*1000)/1000;
        return {q:eq('전체 도수 '+N+', 어떤 계급의 도수 '+f+'일 때 상대도수는? (소수로)'), type:'num', ans:r,
          exp:'상대도수 = 도수 ÷ 전체 도수예요. '+f+' ÷ '+N+' = '+r+'이에요.'};
      }
      // t===5 : 상대도수로 도수 구하기
      var pr5=pick([[20,0.25],[40,0.15],[50,0.2],[25,0.2],[40,0.3],[20,0.4],[50,0.1],[40,0.25]]);
      var N5=pr5[0], r5=pr5[1], f5=Math.round(N5*r5);
      return {q:eq('전체 도수 '+N5+', 어떤 계급의 상대도수 '+r5+'일 때 그 계급의 도수는?'), type:'num', ans:f5,
        exp:'도수 = 상대도수 × 전체 도수예요. '+r5+' × '+N5+' = '+f5+'이에요.'};
    },
    도전:function(){
      var t=ri(1,5);
      if(t===1){ // 백분율
        var pr=pick([[25,4],[20,7],[50,9],[40,14],[25,11],[50,21]]);
        var N=pr[0], f=pr[1], p=Math.round(f/N*10000)/100;
        return {q:eq('전체 '+N+'명 중 어떤 계급의 도수가 '+f+'명이면 전체의 몇 %인가요?'), type:'num', ans:p,
          exp:'먼저 상대도수는 '+f+' ÷ '+N+' = '+(Math.round(f/N*10000)/10000)+'이에요. 100을 곱하면 '+p+'%예요.'};
      }
      if(t===2){ // 히스토그램 넓이
        var s2=pick([2,4,5,10,20]), N2=pick([20,24,25,30,36,40,50]);
        return {q:eq('계급의 크기 '+s2+', 도수의 합 '+N2+'인 히스토그램의 직사각형 넓이의 합은?'), type:'num', ans:s2*N2,
          exp:'넓이 = 계급의 크기 × 도수의 합이에요. '+s2+' × '+N2+' = '+(s2*N2)+'이에요. 도수분포다각형이 감싼 넓이도 같아요.'};
      }
      if(t===3){ // 남은 상대도수
        var A=pick([0.1,0.15,0.2,0.25]), B=pick([0.1,0.2,0.3]), C=pick([0.05,0.1,0.15,0.2]);
        var rest=Math.round((1-A-B-C)*1000)/1000;
        if(rest<=0) { A=0.2; B=0.3; C=0.15; rest=0.35; }
        return {q:eq('네 계급의 상대도수 중 셋이 '+A+', '+B+', '+C+'일 때 나머지 하나는? (소수로)'), type:'num', ans:rest,
          exp:'상대도수를 다 더하면 항상 1이에요. 1 − ('+A+' + '+B+' + '+C+') = '+rest+'이에요.'};
      }
      if(t===4){ // 전체 도수 되짚기
        var pr4=pick([[0.25,6,24],[0.2,7,35],[0.15,6,40],[0.4,8,20],[0.1,5,50],[0.3,9,30]]);
        return {q:eq('상대도수가 '+pr4[0]+'인 계급의 도수가 '+pr4[1]+'일 때 전체 도수는?'), type:'num', ans:pr4[2],
          exp:'전체 도수 = 도수 ÷ 상대도수예요. '+pr4[1]+' ÷ '+pr4[0]+' = '+pr4[2]+'이에요.'};
      }
      // t===5 : 경계값이 들어가는 계급의 계급값
      var s5=pick([5,10,20]), v=pick([20,30,40,50,60,70,80]);
      return {q:eq('계급의 크기가 '+s5+'이고 계급이 '+v+'에서 시작해요. 값 '+v+'이(가) 들어가는 계급의 계급값은?'),
        type:'num', ans:v+s5/2,
        exp:'"'+v+' 이상"은 '+v+'을(를) 포함해요. 그래서 "'+v+' 이상 '+(v+s5)+' 미만"에 들어가고, 계급값은 ('+v+' + '+(v+s5)+') ÷ 2 = '+(v+s5/2)+'이에요.'};
    }
  };
...
,
    ...
    {key:'ds', name:'⑥ 자료 정리', desc:'계급값 · 도수 · 상대도수 — 13단원 수시평가 대비', gen:DS,
     card:'<b>계급값</b> = (아래 끝 + 위 끝) ÷ 2 · <b>계급의 크기</b> = 위 끝 − 아래 끝 · <b>도수</b> = 그 계급에 들어온 개수.<br>'+
          '<b>상대도수</b> = 도수 ÷ 전체 도수 <span class="muted">(다 더하면 1, ×100 하면 %)</span> · 거꾸로 <b>도수 = 상대도수 × 전체</b>, <b>전체 = 도수 ÷ 상대도수</b>.<br>'+
          '<b>히스토그램 넓이</b> = 계급의 크기 × 도수의 합. 이상은 포함, 미만은 제외예요.<br>'+
          '<span class="muted">낱말 뜻이 헷갈리면 📊 <a href="data-mastery.html">자료 읽는 법 특강</a>에서 카드로 풀어 봐요.</span>'}
  ]};
})();

#!/usr/bin/env python3
# render_compile.py — unit-02 알콰리즈미(secondary)
# 6-motion-config.json → scene 클립(zoompan+drawtext+fade) → concat → 7-raw.mp4
# se-video-render §CL/CC 검증 패턴 자동화 (v0.2 컴파일러)
# ★ 입력 = 6-motion-config.json (메인세션 finish_video.py 가 draft 의 rel_start/rel_end 를
#   실측 음성 길이에 매핑해 6-motion-config.json 으로 확정 후 본 스크립트 실행).

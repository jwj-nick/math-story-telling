#!/usr/bin/env python3
# synth.py — unit-02 알콰리즈미(secondary) ElevenLabs 합성 (voice-pool §0.6 절차 재현)
# turn별 합성 → post-process(volume) → concat(silence) → 4-narration.mp3
# voice: Q=Mina(0dB) / A=Mike(+8dB). (실측 매핑·EXECUTE 는 메인세션 finish_video.py)
...
# post-process volume per voice (voice-pool §0.2): Mike +8dB, Mina 없음 (Kyle/Kanna 참고용)
...
def main():
    ...
    for t in turns:
        ...
        # post-process: volume + 재인코딩 44100 mono 128k. 부스트 시 alimiter (voice-pool §0.6)

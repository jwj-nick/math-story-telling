#!/usr/bin/env python3
# render_compile.py — unit-03 viete
# 6-motion-config.json → scene 클립(zoompan+drawtext+fade) → concat → 7-raw.mp4
# se-video-render §CL/CC 검증 패턴 자동화 (v0.2 컴파일러)
# ⚠️ 입력 = 6-motion-config.json (실측 매핑 완료본). 메인세션 finish_video.py 가
#    6-motion-config.draft.json(상대값 0~1) + 음성 길이 → 6-motion-config.json 으로 변환 후 본 스크립트 실행.

#!/usr/bin/env python3
# synth.py — unit-03 viete ElevenLabs 합성 (voice-pool §0.6 절차 재현)
# turn별 합성 → post-process(volume) → concat(silence) → 4-narration.mp3
# pairing = Mina(Q, speed1.15) + Kyle(A, +3.8dB)  ← primary 알콰리즈미(Kanna+Mike)와 다름
...
def main():
    ...
    for t in turns:
        ...
        # ⭐ 부스트 시 alimiter=limit=0.95 (volume 뒤) — 피크 하드클립 방지 (unit-04 학습)

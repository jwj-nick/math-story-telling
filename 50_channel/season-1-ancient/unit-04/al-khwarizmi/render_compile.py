#!/usr/bin/env python3
# render_compile.py — unit-04 알콰리즈미(secondary)
# 6-motion-config.json → scene 클립(zoompan+drawtext+fade) → concat → 7-raw.mp4
# se-video-render §CL/CC 검증 패턴 자동화 (v0.2 컴파일러)
# ★ EXECUTE 전: 6-motion-config.draft.json 의 rel_start/rel_end(0~1) 를
#    합성된 4-narration.mp3 scene 경계(음성 기준)로 절대 초(start/end)로 변환 → 6-motion-config.json 생성 필요.
...
def build_clip(sc):
    ...
    for i, cap in enumerate(sc.get("captions", [])):
        ...
        vf.append(
            ...
            f":fontsize={cap['size']}:x=(w-text_w)/2:y={yexpr}"

#!/usr/bin/env python3
# render_compile.py — unit-08 가우스(작도가능성). 6-motion-config.json → scene 클립(zoompan+drawtext+fade) → concat → 7-raw.mp4
# se-video-render §CL/CC 검증 패턴 자동화 (v0.2 컴파일러). 유클리드 영상 render_compile.py 와 동일 로직(config-driven).

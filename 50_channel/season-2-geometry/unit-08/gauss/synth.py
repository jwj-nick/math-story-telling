#!/usr/bin/env python3
# synth.py — unit-08 가우스(작도가능성) ElevenLabs 합성 (voice-pool §0.6 절차 재현)
# turn별 합성 → post-process(volume) → concat(silence) → 4-narration.mp3
# Q=Mina(여, speed1.15, post 없음) / A=Mike(남, +8dB+리미터). 유클리드 영상(Kanna/Kyle)과 다른 pairing.
...
# post-process volume per voice (voice-pool §0.2): Mike +8dB, Mina 없음

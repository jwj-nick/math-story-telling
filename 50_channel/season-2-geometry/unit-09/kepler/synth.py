#!/usr/bin/env python3
# synth.py — unit-09 케플러 ElevenLabs 합성 (voice-pool §0.6) Q=Mina / A=Kyle
...
# post-process volume per voice (voice-pool §0.2): Kyle +3.8dB, Mina 없음
...
def main():
    ...
    # scene 별 누적 끝 시각도 출력 (motion config 작성용)
    def d(p):
        return float(subprocess.run(["ffprobe","-v","error","-show_entries","format=duration",
            "-of","default=noprint_wrappers=1:nokey=1",str(p)],capture_output=True,text=True).stdout.strip())
    t300, t1000 = d(s300), d(s1000)
    acc = 0.0; prev = None; bounds = {}
    for t in turns:
        if prev is not None:
            acc += t1000 if t["scene"] != prev else t300
        acc += d(NORM / f"turn-{t['id']:03d}.mp3")
        bounds[t["scene"]] = acc  # 각 scene 의 마지막 turn 끝 = scene end
        prev = t["scene"]
    print("SCENE_ENDS " + json.dumps(bounds), flush=True)

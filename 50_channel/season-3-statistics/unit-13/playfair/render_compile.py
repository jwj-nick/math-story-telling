#!/usr/bin/env python3
# render_compile.py — unit-13 플레이페어 / 6-motion-config.json → scene 클립(zoompan+drawtext+fade) → concat → 7-raw.mp4
# se-video-render §CL/CC 검증 패턴 자동화 (v0.2 컴파일러). config-driven, 나이팅게일 템플릿과 동일 로직.
# 주의: EXECUTE 시 6-motion-config.json 의 _draft 를 synth 실측 길이로 갱신한 확정본을 사용할 것.

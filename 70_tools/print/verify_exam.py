"""verify_exam.py — 실전 모의고사 문항 풀(exam.json)·회차 config 검증

사용:
    python 70_tools/print/verify_exam.py u07        # 단원 풀 1개 검증
    python 70_tools/print/verify_exam.py --all      # 13단원 풀 전부
    python 70_tools/print/verify_exam.py --sets     # 6회차 config + 풀 교차 검증

검사 (단원 풀)
  X1 할당표    id·format·level 이 EXAM_SPEC.md 할당표와 정확히 일치
  ...
  X6 교육과정   금지 키워드(제곱근·피타고라스·인수분해 등)
...
검사 (--sets)
  ...
  S4 커버리지  2학기 단원(7~13) 회차당 1~3문항·1학기 합 7~9문항
"""
import json
import re
import sys
...
# EXAM_SPEC.md §5 할당표. (format, [levels]) — id 는 접두사+순번으로 파생된다.
QUOTA = {
    1:  {'cn': 'LMH',  'cv': 'LM',    'sh': 'L',  'es': 'LM'},
    2:  {'cn': 'LMH',  'cv': 'LM',    'sh': 'L',  'es': 'MH'},
    3:  {'cn': 'LMHH', 'cv': 'LM',    'sh': '',   'es': 'LM'},
    4:  {'cn': 'LMMH', 'cv': 'LM',    'sh': '',   'es': 'MH'},
    5:  {'cn': 'LMH',  'cv': 'LM',    'sh': 'L',  'es': 'LM'},
    6:  {'cn': 'LMH',  'cv': 'LM',    'sh': 'L',  'es': 'MH'},
    7:  {'cn': 'LMMH', 'cv': 'LLMH',  'sh': 'LM', 'es': 'LM'},
    8:  {'cn': 'LMMH', 'cv': 'LLMH',  'sh': 'LM', 'es': 'MH'},
    9:  {'cn': 'LMMH', 'cv': 'LLMHH', 'sh': 'LM', 'es': 'H'},
    10: {'cn': 'LMMH', 'cv': 'LLMH',  'sh': 'LM', 'es': 'MH'},
    11: {'cn': 'LMMH', 'cv': 'LLMHH', 'sh': 'LM', 'es': 'H'},
    12: {'cn': 'LMMH', 'cv': 'LLMH',  'sh': 'LM', 'es': 'MH'},
    13: {'cn': 'LMMH', 'cv': 'LLMH',  'sh': 'LM', 'es': 'LM'},
}
...
BANNED = ['제곱근', '루트', '√', '피타고라스', '인수분해', '이차방정식', '삼각비', 'sin', 'cos', 'tan']
...
def load_bank(unit):
    p = BANK_ROOT / 'math1' / ('u%02d' % unit) / 'practice.json'
...
def check_unit(unit):
    path = BANK_ROOT / 'math1' / ('u%02d' % unit) / 'exam.json'
    if not path.exists():
...
def check_sets():
    pool = load_pool()
    cfgs = sorted(SETS_DIR.glob('m1-exam-*.json'))
    if len(cfgs) != 6:
    ...
    for cp in cfgs:
        ...
        for u in range(7, 14):
            if not 1 <= units_used.get(u, 0) <= 3:
                errs.append('S4 %s 단원 %d 문항 수 %d (1~3 범위 밖)' % (name, u, units_used.get(u, 0)))
